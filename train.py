...
with tf.Session(config=config_gpu) as sess:
    ...
    writer = tf.summary.FileWriter(params['tsboard_save_path'], sess.graph)
    init = tf.global_variables_initializer()
    sess.run(init)

    num_iters = np.int32(45000 / batch_size) + 1
    print('Start training...')
    for epoch in range(train_ep):
        print('Eopch %d'%epoch)
        for iters in range(num_iters):
            if iters % val_step_iter == 0 and iters != 0:
                # run validation, feed a batch_size each time
                next_images, next_labels = dataset.get_val_batch()
                val_feed_dict = {train_img: next_images, train_label:
                                 next_labels}
                [val_accuracy_, total_loss_,Val_summary_] = sess.run([train_acc,total_loss,Val_summary], val_feed_dict)
                writer.add_summary(Val_summary_, iters)
