...
def ResUnit_downsample_2convs(input_tensor, feed_dict, shape_dict,
    ...
    with tf.variable_scope('conv2'):
        CONV_out2 = conv_layer(RELU_out2, feed_dict, 1, 'SAME',
...
def ResUnit_2convs(input_tensor, feed_dict, shape, var_dict=None):
    ...
    # The first conv layer
    with tf.variable_scope('conv1'):
        CONV_out1 = conv_layer(RELU_out1, feed_dict, 1, 'SAME', shape, var_dict)
    # The second batch norm layer
    BN_out2 = BN(CONV_out1, feed_dict, 'bn2', is_train, shape[2], var_dict)
    RELU_out2 = ReLu_layer(BN_out2)
    # The second conv layer
    with tf.variable_scope('conv2'):
        CONV_out2 = conv_layer(RELU_out2, feed_dict, 1, 'SAME', shape, var_dict)
...
def conv_layer(input_tensor, feed_dict, stride, padding='SAME', shape=None,
    ...
    conv_out = tf.nn.conv2d(input_tensor, kernel, strides=[1, stride, stride,
                                                           1], padding=padding)
    if var_dict is not None:
        if not var_dict.has_key(scope_name):
            var_dict[scope_name] = {}
...
def BN(input_tensor, feed_dict=None, bn_scope=None,is_train=False, shape=None,
    ...
    BN_out = tf.layers.batch_normalization(inputs=input_tensor,
                                          ...
                                          training=is_train, # train or test
                                          trainable=True, # must always true
...
def Global_avg_pool(input_tensor):
    '''Using reduce_mean to perform global avg pool'''
...
def FC(input_tensor, batch_size, feed_dict, num_class, var_dict=None):
    ...
    if var_dict is not None:
        if not var_dict.has_key(scope_name):
            var_dict[scope_name] = {}
        var_dict[scope_name]['fc_weight'] = fc_weight
        var_dict[scope_name]['fc_bias'] = fc_bias

    return fc_out

def get_fc_weight(feed_dict, shape):
...
def get_fc_bias(feed_dict, shape):
    ...
    if not feed_dict.has_key(scope_name):
        ...
        shape = init_val.shape
        print('Load fc_bias with shape: %s' % str(shape))
        init = tf.constant_initializer(value=init_val)
